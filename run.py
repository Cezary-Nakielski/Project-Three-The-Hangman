...
tried '{guess}'.{Colours.RESET}\n")
            elif guess not in secret_word:
                attempt_letter.append(guess)
                attempts -= 1
                print(f"{Colours.RED}'{guess}' is not contained in \
the word.{Colours.RESET}\n")
            else:
                attempt_letter.append(guess)
                ...
                print(f"{Colours.GREEN}'{guess}' is contained \
in the word.{Colours.RESET}\n")
                if "_" not in word_display:
                    game_score = True
        elif len(secret_word) == len(guess) and guess.isalpha():
            if guess in attempt_word:
                print(f"{Colours.RED}You already tried the \
word '{guess}' before{Colours.RESET}.\n")
...
you're looking for.{Colours.RESET}\n")
            ...
            print(f"{Colours.RED}It must be a letter or \
the whole word.{Colours.RESET}\n")
        print("  " + " ".join(word_display) + "\n")
        print(f"Attempts: {attempts}\n")
